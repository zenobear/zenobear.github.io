/* ==========================================================================
   1. DATA LAYER (LOCALSTORAGE / FIREBASE-READY)
   ========================================================================== */
const DB = {
    ...
    initSeedData: () => {
        if (!localStorage.getItem('products')) {
            const defaultProducts = [
                { id: 'p1', name: 'Zeno Bear Onesie', sku: 'ZB-ONE-01', barcode: '123456789', category: 'Baby Clothing', price: 450, cost: 250, stock: 25, minStock: 5, status: 'Active' },
                { id: 'p2', name: 'Zeno Bear Graphic Tee', sku: 'ZB-TEE-01', barcode: '111222333', category: 'Apparel', price: 380, cost: 190, stock: 18, minStock: 5, status: 'Active' },
                { id: 'p3', name: 'Zeno Plush Toy', sku: 'ZB-PLS-01', barcode: '444555666', category: 'Toys', price: 550, cost: 280, stock: 8, minStock: 3, status: 'Active' },
                { id: 'p4', name: 'Zeno Bear Hoodie', sku: 'ZB-HOD-01', barcode: '777888999', category: 'Apparel', price: 850, cost: 450, stock: 3, minStock: 5, status: 'Active' }
            ];
...
const SalesLogic = {
    ...
    recordSale: (cart, paymentDetails) => {
        ...
        const txnNumber = `ZB-${YYYY}${MM}${DD}-${seq}`;

        const saleRecord = {
            id: txnNumber,
            businessName: "Zeno Bear",
...
const POS = {
        ...
        document.getElementById('posSearch').addEventListener('keypress', (e) => {
            if (e.key === 'Enter') {
                ...
                if (matched) {
                    ...
                    showToast('No matching active Zeno Bear product found');
                }
            }
        });
    },
    ...
    renderProducts: (filter = '') => {
        ...
        if (filtered.length === 0) {
            grid.innerHTML = `<div style="grid-column: 1/-1; text-align: center; color: var(--text-muted); padding: 2rem;">No matching active products found</div>`;
            return;
        }
    ...
    renderCart: () => {
        ...
        if (POS.cart.length === 0) {
            list.innerHTML = `<div style="text-align: center; color: var(--text-muted); margin: auto; padding: 2rem;">Cart is empty</div>`;
    ...
    finalizeSale: () => {
        ...
        POS.showReceiptModal(txn);
        POS.clearCart();
        POS.renderProducts();
    },

    showReceiptModal: (txn) => {
        document.getElementById('receiptTxnId').innerText = txn.id;
        document.getElementById('receiptDate').innerText = `Date: ${txn.date} ${txn.time}`;
        document.getElementById('receiptMethod').innerText = txn.paymentMethod;
        document.getElementById('receiptPaid').innerText = txn.amountPaid.toFixed(2);
        document.getElementById('receiptChange').innerText = txn.change.toFixed(2);
        document.getElementById('receiptTotal').innerText = txn.total.toFixed(2);

        const itemsContainer = document.getElementById('receiptItemsList');
        itemsContainer.innerHTML = txn.items.map(i => `
...
const Admin = {
    ...
    renderDashboard: () => {
        ...
        if (topSellers.length === 0) {
            topTbody.innerHTML = `<tr><td colspan="4" style="text-align:center; color:var(--text-muted);">No Zeno Bear sales recorded yet</td></tr>`;
        } else {
    ...
    saveProduct: (e) => {
        ...
        Admin.refresh();
        POS.renderProducts();
        showToast("Zeno Bear product saved!");
    },
